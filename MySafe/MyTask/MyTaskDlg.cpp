...
BEGIN_MESSAGE_MAP(CMyTaskDlg, CDialogEx)
	...
	ON_WM_HOTKEY()
	ON_BN_CLICKED(IDC_RADIO6, &CMyTaskDlg::OnBnClickedRadio6)
	ON_BN_CLICKED(IDC_RADIO7, &CMyTaskDlg::OnBnClickedRadio7)
	...
	ON_BN_CLICKED(IDC_RADIO12, &CMyTaskDlg::OnBnClickedRadio12)
	ON_WM_CLOSE()
	ON_COMMAND(ID_DEBUG, &CMyTaskDlg::OnDebug)
...
{
	hModule = LoadLibrary(TEXT("MySafeSkin.dll"));
	if (hModule)
	{
...
void CMyTaskDlg::OnDebug()
{
	...
	_stscanf_s(nPidTemp, TEXT("%d"), &nPid);
	DebugDlg *pDebugDlg = new DebugDlg(nProName, nPid);
...
void CMyTaskDlg::OnBnClickedRadio1()
{
	...
	if (m_PPeDlg != nullptr)m_PPeDlg->ShowWindow(SW_HIDE);
	if (m_PServiceDlg != nullptr)m_PServiceDlg->ShowWindow(SW_HIDE);
	...
	if (m_PTcpDlg != nullptr)m_PTcpDlg->ShowWindow(SW_HIDE);
...
//**********************************显示PE解析**********************************
void CMyTaskDlg::OnBnClickedRadio6()
{
	if (m_PPeDlg == nullptr)
	{
		CRect nHeap;
		m_PPeDlg = new MyPeDlg;
		m_PPeDlg->SetParent(this);
		m_PPeDlg->GetClientRect(nHeap);
		nHeap.top = 33;
		nHeap.left = 11;
		m_PPeDlg->MoveWindow(nHeap);
	}

	m_PPeDlg->ShowWindow(SW_SHOW);
...
//**********************************显示系统服务**********************************
void CMyTaskDlg::OnBnClickedRadio7()
{
	if (m_PServiceDlg == nullptr)
	{
		CRect nHeap;
		m_PServiceDlg = new MyServiceDlg;
		m_PServiceDlg->SetParent(this);
		m_PServiceDlg->GetClientRect(nHeap);
		nHeap.top = 33;
		nHeap.left = 11;
		m_PServiceDlg->MoveWindow(nHeap);
	}

	m_PServiceDlg->ShowWindow(SW_SHOW);
...
//**********************************显示网络监控**********************************
void CMyTaskDlg::OnBnClickedRadio12()
{
	if (m_PTcpDlg == nullptr)
	{
		CRect nHeap;
		m_PTcpDlg = new MyTcpDlg;
		m_PTcpDlg->SetParent(this);
		m_PTcpDlg->GetClientRect(nHeap);
		nHeap.top = 33;
		nHeap.left = 11;
		m_PTcpDlg->MoveWindow(nHeap);

	}

	m_PTcpDlg->ShowWindow(SW_SHOW);
	...
	if (!g_OpenTraffic)
	{
		if (pMyTrafficDlg != nullptr)
		{
			delete pMyTrafficDlg;
			pMyTrafficDlg = nullptr;
		}
		g_OpenTraffic = !g_OpenTraffic;
		pMyTrafficDlg = new MyTrafficDlg;
	}
