...
void MyAntiVirusDlg::ScanFile(CString dir)
{
	...
	do {
		if (m_IsScaning == FALSE)return;

		if (CString(fData.cFileName) == L"." || CString(fData.cFileName) == L"..")continue;
		...
		else
		{
			...
			{
				if (strcmp(m_VirusInfo[i].FileMd5, nFileMd5) == 0)//如果MD5吻合
				{
...
void MyAntiVirusDlg::OnBnClickedButton1()
{
	...
	if (m_Check_Cloud.GetCheck()) UpDataCloud();
...
{
	m_Button_Start_Process.EnableWindow(FALSE);
