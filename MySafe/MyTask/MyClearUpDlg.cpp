...
//清理查找记录
VOID MyClearUpDlg::CleanFileFindHistory()
{
	if (IsScanFile)
		return;
...
DWORD WINAPI ScanClearUpThread(LPVOID lpThreadParameter)
{
	...
	if (nDlg->m_Check_FindHistory.GetCheck())nDlg->CleanFileFindHistory();//清理查找记录
	...
	if (nDlg->m_Check_AddrHistory.GetCheck())nDlg->CleanAddrBarHistory();//清理浏览器地址记录
